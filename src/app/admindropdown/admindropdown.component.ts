...
@Component({
  selector: 'app-admindropdown',
  standalone: true,
  imports: [FormsModule, CommonModule, HttpClientModule, AdminbookroomComponent],
  templateUrl: './admindropdown.component.html',
  styleUrl: './admindropdown.component.css'
})
export class AdmindropdownComponent {
   phases: any[] = [];
    ...
    constructor(private phaseService: PhaseService, private bookingService: RoomService,private router: Router) {
        ...
        .subscribe(
          (response: any) => {
            if (response && response.message && response.message === "No Access") {
              ...
              this.router.navigate(['/adminbookingg'],{ state: { meetingRooms: this.meetingRooms } });
            }
          },
          (error) => {
            console.error(error);
