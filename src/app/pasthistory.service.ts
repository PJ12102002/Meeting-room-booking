...
export class PasthistoryService {
  constructor(private http: HttpClient) {}
  //apiURL: string = "http://localhost:3000";
  getPastHistory(): Observable<any> {
    return this.http.get<any>(`http://localhost:8080/admin/bookHistory`); }
     getCurrentHistory(): Observable<any> {
      return this.http.get<any>(`http://localhost:8080/admin/currentHistory`);
    }
  ...
  cancelBooking(bookId: number): Observable<any> {
    ...
    return this.http.get<any>(`http://localhost:8080/admin/cancelRoom/${bookId}`,{});
